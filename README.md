...
### 2.1.0
- 兼容 GD (GundamIT) 属性结构：标签搜索支持别名映射（Theme↔Theme[IP]、Company↔Brand）
- 新增 Hot Categories AI 匹配（GD 独有属性）
- SZ 独有属性（Product Type、Character、Featured In）在 GD 上自动跳过
- 提取 `findAttrContainer` 通用标签搜索函数，统一 Availability 和 AI 属性的查找逻辑
